...
# try:
#     from code_analysis import ASTReader, AST_fragmentation, fragmentTypes, AST
# except ImportError:
#     print("Erreur: Le module 'code_analysis' n'a pas été trouvé.")
#     print("Assurez-vous qu'il est dans le PYTHONPATH ou le répertoire courant.")
#     sys.exit(1)

# Configuration
path_ast = "ast"  # Chemin vers le dossier contenant les kits d'AST
MIN_NODES_FILE = 100
...
def get_function_name(ast, fragment_node_id):
    ...
    # Stratégie simple : chercher un enfant de type 'Id' qui pourrait être le nom
    try:
    ...
    except Exception: # Ignorer les erreurs si la structure est inattendue
        pass
...
reader = ASTReader()
kit_files_map, kits = get_ast_files(path_ast)
...
# Initialiser un AST pour obtenir la taille du vecteur
try:
...
for kit_id in kits:
    ...
    for filepath in kit_files_map[kit_id]:
        try:
            ...
            # 1. Vecteur du fichier complet
            file_vector = ast.vectorize()
            file_node_count = int(file_vector.sum()) # Utiliser int() pour compatibilité hash
            if file_node_count > 0: # Ignorer fichiers vides si jamais
                 all_files_data.append((filepath, kit_id, file_vector, file_node_count))

            # 2. Fragmentation et vecteurs des fragments
            fragment_nodes = AST_fragmentation(ast) # Note: Modifie l'AST en coupant les liens parents
            
            # Recharger l'AST pour l'analyse des fragments si nécessaire ou dupliquer avant fragmentation
            # Ici, on suppose qu'on peut travailler sur l'AST fragmenté pour obtenir les vecteurs de fragments
            # MAIS il faut être conscient que l'AST global n'est plus le même.
            # Pour être sûr, rechargeons l'AST original pour l'extraction des fragments
            ast_for_fragments = reader.read_ast(filepath) # Relire pour avoir l'AST complet

            for fragment_node_id in fragment_nodes:
                 print("Noeud fragment :", fragment_node_id)
                 # Vectoriser le sous-arbre à partir du noeud fragment
...
# --- Analyse Section 1: Fichiers ---

print("\n--- Section 1: Analyse au niveau des fichiers ---")

# 1.1 Clones "paramétriques" (Fichiers)
...
for filepath, kit_id, vector, _ in files_filtered:
    vector_tuple = tuple(vector) # Numpy arrays ne sont pas hashables
...
if largest_group_files_exact:
    # Afficher quelques exemples de fichiers dans le plus grand groupe
...
vectors_files_exact_unique_kit = collections.defaultdict(set) # Utilise un set pour stocker les kit_id uniques
for filepath, kit_id, vector, _ in files_filtered:
...
# Utiliser les fichiers déjà filtrés par taille (files_filtered)
n_files = len(files_filtered)
similarity_groups = collections.defaultdict(list) # clé=index_fichier, valeur=liste index fichiers similaires
...
if n_files > 1:
    ...
    for i in range(n_files):
        ...
        # Ajouter le fichier lui-même à son groupe
        similarity_groups[i].append(i)
        # Seuil pour ce fichier
        ...
        for j in range(i + 1, n_files):
            ...
            # Vérifier la similarité (on peut vérifier le seuil par rapport à i ou j)
            # Prenons la condition la plus souple: si similaire par rapport à i OU j
            ...
            if distance <= threshold or distance <= threshold_j:
                 ...
                 similarity_groups[j].append(i) # La relation est symétrique
else:
    print("Pas assez de fichiers (>1) pour comparer la similarité.")

# Trouver le plus grand groupe basé sur le nombre de voisins similaires
largest_group_files_similar_indices = []
if similarity_groups:
    # Trouver l'index du fichier ayant le plus de voisins similaires
...
if largest_group_files_similar_indices:
    ...
    # Récupérer les infos des fichiers correspondants aux indices
...
processed_frag_kit_vector = set() # Pour s'assurer qu'on ne traite qu'un seul fragment identique par kit

for filepath, kit_id, frag_id, vector, count, name in fragments_filtered:
    vector_tuple = tuple(vector)
    # Clé unique pour ignorer les doublons exacts dans le même kit pour *ce* vecteur
    ...
    if frag_kit_vector_key not in processed_frag_kit_vector:
        ...
        if name: # Ajouter le nom si trouvé
             vectors_fragments_exact_unique_kit[vector_tuple]['names'].add(name)
        # Garder un exemple de fichier/id pour référence
...
# Trouver le vecteur (fragment) présent dans le plus grand nombre de kits différents
most_duplicated_fragment_info = None
max_kits_count = 0
if vectors_fragments_exact_unique_kit:
    # Trouver l'entrée (vecteur) avec le plus grand nombre de kits uniques
...
# Utiliser les fragments déjà filtrés par taille (fragments_filtered)
# Étape 1: Créer une liste de fragments où chaque (vecteur, kit_id) est unique.
unique_fragments_per_kit = {} # clé: (vector_tuple, kit_id), valeur: (vector, kit_id, name, filepath)
for filepath, kit_id, frag_id, vector, count, name in fragments_filtered:
    vector_tuple = tuple(vector)
    key = (vector_tuple, kit_id)
    if key not in unique_fragments_per_kit:
         # Garder le premier rencontré pour cette combinaison vecteur/kit
...
# Étape 2: Calculer les similarités par paires sur cette liste unique
similarity_groups_frags = collections.defaultdict(list) # clé=index_frag, valeur=liste index frags similaires
...
if n_fragments > 1:
    ...
    for i in range(n_fragments):
        ...
        similarity_groups_frags[i].append(i) # Ajouter soi-même
        ...
        for j in range(i + 1, n_fragments):
            ...
            # Important: Assurer que les kits sont différents pour la similarité inter-kit
...
if largest_group_frags_similar_indices:
    ...
    for idx in largest_group_frags_similar_indices[:5]:
...
# --- Analyse Section 3: Kits ---

print("\n--- Section 3: Analyse au niveau des kits 'paramétriques' ---")
start_time = time.time()

# Calculer un vecteur représentatif pour chaque kit en sommant les vecteurs de ses fragments
...
# Chercher le plus gros groupe de vecteurs de kits identiques
kit_vector_groups = collections.defaultdict(list)
for kit_id, sum_vector in kit_vectors_sum.items():
    # Ignorer les kits sans fragments (vecteur nul)
    if sum_vector.sum() > 0:
...
print(f"\nAnalyse 3 terminée en {time.time() - start_time:.2f} secondes.")
print("-" * 30)
